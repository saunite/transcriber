...
# (openspec/changes/add-tauri-gui). Produces the Windows portable build via
# GNU/mingw-w64 cross-compilation (no MSVC/Visual Studio/Wine needed) and
# the Linux AppImage natively, from one Linux-based image -- built to work
# around this machine's execution-policy block on Rust toolchain binaries
# (see openspec/changes/add-tauri-gui/tasks.md). No installer bundling
# (NSIS/.deb) as of openspec/changes/remove-installer-packaging -- both
# platforms ship a single no-install artifact instead.
