...
- **Checks that need a published release.** `v0.1.0` was published on 2026-09-17, and the in-app **Check for updates** reports it as up to date. `tests/test_e2e_linux.py`'s `update check online` also read "You're up to date (0.1.0)" on 2026-09-17, so that sanity check is done.
  1. **`add-manual-update-check` 4.3, the "update available" path against the real API.** Build a temporary debug app whose `tauri.conf.json` version is lower than the published one (e.g. `0.0.1`), or wait for the next release. Clicking **Check for updates** must show the newer version, and **Open download page** must open `https://github.com/saunite/transcriber/releases/latest` in the browser. Record the method in its archive, `openspec/changes/archive/2026-09-15-add-manual-update-check/tasks.md`.
  2. **`add-linux-package-repos` 6.4, the upgrade half, at the next release.** After publishing it, confirm `apt` and `dnf` offer and install the new version on a machine that has v0.1.0 from the `.deb` or `.rpm`. Record the result in `openspec/changes/archive/2026-09-16-add-linux-package-repos/tasks.md`.
...
- **WASAPI capture only mixes 2-channel output to mono.** `WASAPICapture.capture_stream` opens the loopback at the device's `maxInputChannels` and reshapes only when that is 2 (`wasapi_capture.py`). A 5.1 or 7.1 output device would feed interleaved samples straight to transcription as if they were mono. Found during the ponytail audit on 2026-09-14 and deliberately left out of `01-merge-dual-capture-paths`. Today a 5.1 or 7.1 output is transcribed as garble: six or eight interleaved channels are fed in as one, at six or eight times the real rate.

  **Decided on 2026-09-17, before any implementation:** the fix SHALL let the user choose which channels are used, and SHALL NOT assume them. Averaging every channel can cancel out-of-phase content and pulls in the LFE; silently picking the centre or the front pair is a guess about the user's content that the user cannot see or override. Whatever the default is, it must be visible and changeable. So this needs a user-facing setting, not only `reshape(-1, CHANNELS).mean(axis=1)`: a GUI control (which goes through the design pass in DESIGN.md, the finish review, the detector and the screenshots) plus the equivalent CLI flag, since the app is how Windows live capture is normally used. Neither the mixdown nor the alternative can be tested here: no 5.1 hardware, so a synthetic interleaved buffer is the only local check.
