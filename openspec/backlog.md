...
- **Logic audit follow-ups (2026-09-15).** A read-through of every product module against its specs, comments and UI promises. Cluster A (engine liveness) was done in the archived `fix-engine-liveness`, and cluster B (refused starts) in `fix-refused-start-routing`. The rest were read in the code but not reproduced. Each cluster should become one change that starts with a test failing today.
