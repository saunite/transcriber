...
- **macOS release pipeline, in progress.** Change `03-add-release-pipeline-macos` is at 3/8. The helper guard, ad-hoc signing, launcher, `macos` job and README are written; 1.1, 1.2 and 2.1–2.3 can only be verified by a macOS CI run, which needs the user's go-ahead. Nothing macOS-specific has run on real hardware: there is no Mac.
