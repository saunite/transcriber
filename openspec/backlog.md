...
- **Checks that need a published release.** `v0.1.0` was published on 2026-09-17, and the in-app **Check for updates** reports it as up to date.
  ...
  3. **The e2e online verdict should now read "You're up to date (…)".** `tests/test_e2e_linux.py`'s `update check online` accepted "No releases published yet" before the release; it accepts either, so this is only a sanity check at the next e2e run. Some VPNs drop GitHub's 140.82.112.0/22 range, and the scenario then prints `SKIP`.
