...
- **Logic audit follow-ups (2026-09-15).** A read-through of every product module against its specs, comments and UI promises. Cluster A (engine liveness) was done in the archived `fix-engine-liveness`, cluster B (refused starts) in `fix-refused-start-routing`, and cluster C (the time axis) in `fix-true-scale-time-axis`. From cluster D, D1, D5, D7 and the Linux/macOS half of D4 were done in `01-fix-audit-edges`. The rest were read in the code but not reproduced. Each cluster should become one change that starts with a test failing today.
