...
- [ ] 6.1 Rebuild and capture screenshots into `.impeccable/review/` (desktop, plus the app's minimum 640x480), validating each capture actually shows what its filename claims

  **BLOCKED — no capture capability in this environment.** No chromium/firefox, no Playwright, and no `import`/`scrot`/`grim`/`gnome-screenshot`; installing any needs `sudo`, which this session's shell cannot authenticate. The Windows artifact has been rebuilt and reassembled so the render can be seen on the real target platform. Needs either user-supplied screenshots or a sudo install.
...
- [ ] 6.3 Spawn `impeccable-finish-reviewer` with the request, artifact paths, screenshots, direction contract, and detector findings; act on its disposition word

  **Blocked by 6.1** — the reviewer has no browser, so screenshots are checks it cannot run without them.
- [ ] 6.4 Spawn `impeccable-documenter` to replace `DESIGN.md` from the built world — the existing "Verbatim" DESIGN.md describes a world this change removes

  Runs after the last correction lands, per the documenter's own sequencing rule.
