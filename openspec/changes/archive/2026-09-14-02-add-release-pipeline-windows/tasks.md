...
- [x] 4.2 Uninstall through Windows settings. Confirm the app files and the Start Menu entry are gone with no admin prompt, apart from the WebView2 cache already documented in `05` task 2.5.

  **User-verified 2026-09-14** (Windows 11, CI build from run 34859698393). Uninstalling through Windows settings, with "Delete the application data" ticked, showed no prompt and no request for administrator rights. Afterwards `AppData\Local\Transcriber` was gone, and so were the Start Menu entry and the desktop icon.
...
- [x] 4.4 Extract the CLI zip and run `transcriber.exe --file <clip>` with the network disconnected. It must transcribe with the bundled model. Then run `win-start-transcription.bat` and confirm a live session starts from the bundled exe.
  ...
  **Partial, 2026-09-14** (Windows 11, same build):
  ...
  - **Re-tested after 4.5, user-verified 2026-09-14:** with the updated `win-start-transcription.bat` in the extracted CLI folder, a live session started from the bundled `transcriber.exe` and saved its transcript correctly. **Passes.**
- [x] 4.5 In `win-start-transcription.bat`, replace the `wmic os get localdatetime` timestamp with `powershell -NoProfile -Command "Get-Date -Format yyyyMMdd_HHmmss"`, keeping the `<prefix>_YYYYMMDD_HHMMSS.txt` name that `teams-launcher` requires. Verify on Windows that `win-start-transcription.bat TEST` from the extracted CLI zip names the file `TEST_<YYYYMMDD>_<HHMMSS>.txt`, that the file holds the printed transcript lines after stopping, and that no `wmic` error appears. Then complete 4.4.

  **Implemented 2026-09-14; Windows verification open.** The `wmic` loop is replaced with `for /f %%I in ('powershell -NoProfile -Command "Get-Date -Format yyyyMMdd_HHmmss"') do set timestamp=%%I`. `timestamp` is cleared first, and **beyond the task text**, if PowerShell yields nothing the script now prints `Could not read the current time from PowerShell…` and exits 1, instead of building a broken filename silently. A silent broken name was how the original bug went unnoticed. The name keeps the `<prefix>_YYYYMMDD_HHMMSS.txt` shape of the Linux and macOS launchers' `date +%Y%m%d_%H%M%S`. `transcribe_file.bat` never used `wmic`, so it is unchanged.
  **Checked without Windows:** the new block was extracted and run under Wine's `cmd` with a stub `powershell.bat` printing `20260914_153012`: output `TEST_20260914_153012.txt`, exit 0. With no `powershell` on the path it printed the error and exited 1. Wine has no real PowerShell, so the actual `Get-Date` output still needed the Windows run.
  **User-verified on Windows 11, 2026-09-14:** "The new version of the .bat file worked." No `wmic` error appeared, and the file was named `TEST_<YYYYMMDD>_<HHMMSS>.txt` with the transcript inside.
