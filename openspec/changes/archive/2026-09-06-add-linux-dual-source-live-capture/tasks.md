...
- [x] 3.1 Run a real dual-source live session on this machine (real mic + PipeWire monitor), speak into the mic while system audio plays, confirm both sources transcribe with correct tags and the merged WAV plays back correctly on each channel

  **Fully verified — confirmed by the user.** The SYS-side half was already re-verified for real after `fix-linux-loopback-detection` landed (real `[SYS]`-tagged transcribed segment from played speech). The remaining piece — a human actually speaking into the mic concurrently with system audio — was tested directly by the user, who confirmed the microphone worked. Combined with 1.3's already-verified byte-level merged-WAV mechanics, this closes out the last open item for this change.
