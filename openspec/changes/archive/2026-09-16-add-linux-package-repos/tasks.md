...
- [ ] 6.4 **At the first published release, by the user:** on a real Debian or Ubuntu machine and a real Fedora or openSUSE machine, install the downloaded package, confirm the repository registered itself, then publish a later version and confirm the package manager offers and installs it. Record both results; this is the check the container tests cannot make.
  ...
  **Not done (archived open, 2026-09-16).** It needs a published release, and nothing is published yet. Everything it would prove was verified locally instead: the indexes were built from the real packages, `apt` offered the package through GitHub's redirect shape, `dnf` listed it from signed Pages metadata, and the container suite covers signature refusals and the upgrade behaviour of the repository file. What remains unproven is the live path: the publish workflow running on GitHub, the real signing key, the `gh-pages` push, and `releases/latest/download/` resolving. The user chose the first real release as that test.

  **Live path verified at v0.1.0 (2026-09-17); upgrade half still open.** The user published v0.1.0, and the `publish repositories` workflow (run `35179762079`) signed with the real subkey, uploaded `InRelease`, `Packages` and `Packages.gz` to the release, and pushed `rpm/repodata/` to `gh-pages`. The CI-built `.deb` lists `/etc/apt/sources.list.d/transcriber.list` in `conffiles`, and the `.rpm` marks `/etc/yum.repos.d/transcriber.repo` as `%config(noreplace)`. The user installed the `.rpm` on Fedora and it worked, including **Check for updates**. From clean containers against the public URLs: `debian:12` fetched the signed index through `releases/latest/download/` and offered `Candidate: 0.1.0`; `fedora:42` imported key `0xB40BC439` at the one-time prompt, listed `transcriber.x86_64 0.1.0-1`, and resolved the payload to the v0.1.0 release asset. Pages served `repomd.xml` about fifteen seconds after the push, so the dnf side briefly lags the apt side. Still unproven: that a later release is offered and installed as an upgrade. That check waits for the next release, so the box stays open.
