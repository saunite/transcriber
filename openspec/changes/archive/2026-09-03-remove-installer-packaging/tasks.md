...
- [x] 2.6 Run `build_portable.py` on Windows and confirm the produced zip, extracted to a fresh path, runs and transcribes — the same end-to-end check `add-portable-build` did on the folder

  **Superseded — moved to `05-remove-installer-packaging-windows`.** Per `01-adopt-platform-split-requirements`, Windows-only outstanding verification for this change was split into its own change so this change's general/Linux scope can archive independently. See that change's tasks.md task 2.1.

  Prior partial verification kept for reference: **Partially verified, real Windows execution still needed.** `build_portable.py`'s Windows path ran for real in CI (run 33706036745, windows-latest, MSVC target) against a genuine `cargo tauri build` output plus the real frozen sidecar and real `base` model (not stubs) -- downloaded and inspected the resulting `Transcriber.zip`: correct `Transcriber/{transcriber-gui.exe, transcriber-sidecar.exe, resources/model/{model.bin, config.json, tokenizer.json, vocabulary.txt}}` layout, `WebView2Loader.dll` correctly absent (statically linked on MSVC, per task 1.2's finding) and correctly not required. What's not verified: actually running the extracted `.exe` on Windows and transcribing something -- this machine is Linux and cannot execute a Windows GUI binary. Same class of gap as `drop-ffmpeg-dependency` task 3.4: needs Windows hardware.
...
- [x] 4.2 Build for Windows and double-click the exe from Explorer — confirm the app window appears with no console window before, beside, or behind it

  **Superseded — moved to `05-remove-installer-packaging-windows`** (task 2.2). Per `01-adopt-platform-split-requirements`.
- [x] 4.3 Start a live session and a file transcription and confirm no console window flashes when the sidecar spawns. `tauri-plugin-shell` 2.3.6 already sets `CREATE_NO_WINDOW` (design.md Decision 5), so this is a confirmation, not a fix — and do **not** rebuild the sidecar with PyInstaller `--noconsole`, which would break running it directly from a terminal

  **Superseded — moved to `05-remove-installer-packaging-windows`** (task 2.3). Per `01-adopt-platform-split-requirements`.
...
- [x] 4.5 Confirm double-clicking `Transcriber.app` in Finder opens no Terminal window

  **Superseded — moved to `06-remove-installer-packaging-macos`** (task 2.1), parked pending macOS hardware/CI. Per `01-adopt-platform-split-requirements`.
...
- [x] 5.1 Windows: extract the zip on a machine that has never had the app installed, run `transcriber-gui.exe`, confirm no admin prompt, no registry writes, and a working transcription

  **Superseded — moved to `05-remove-installer-packaging-windows`** (task 2.4). Per `01-adopt-platform-split-requirements`.
