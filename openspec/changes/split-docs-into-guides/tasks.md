...
- [ ] 2.1 Write the rest of `CONTRIBUTING.md` (new text), covering:
...
- [ ] 2.2 Rewrite `README.md` short (target: about 120 lines), in this order: icon and one-paragraph description; what it does (trimmed features); **Install** (desktop app per platform, from today's "Download and run", keeping the macOS testers call and the unsigned-binary notes); **Run it** (the app in a few steps, then the CLI in a few lines); **Where next** (links to `docs/user-guide.md`, `CONTRIBUTING.md`, `docs/building.md`); **License** (kept verbatim: the `licensing` capability names `LICENSE` and `README`).
