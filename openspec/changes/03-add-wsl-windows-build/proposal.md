...
- Freeze the Windows sidecar via interop: invoke the existing Windows Python venv's interpreter (`./.venv/Scripts/python.exe build_sidecar.py`) from WSL — WSL can execute Windows `.exe` binaries directly, and this venv already has every sidecar dependency installed, confirmed working. Guard this step by probing `shutil.which("python.exe")` (or equivalent), not by sniffing the host OS, so it fails loudly with a clear message when no Windows Python is reachable (e.g. on a real Linux machine, where this whole change's Windows-specific tasks don't apply).
