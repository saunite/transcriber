...
**Probe for the interpreter, don't sniff the OS.** `platform.system()` inside a WSL process reports `"Linux"`, never `"Windows"` — there is no OS-sniff that correctly answers "is a Windows Python reachable from here." The correct check is `shutil.which("python.exe")` (or an equivalent explicit interpreter-path check): true only when a Windows Python is actually on `PATH`/reachable, false on a real Linux machine with no interop path — which is exactly the condition this step needs to gate on.
